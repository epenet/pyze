...
async def wrap_unavailable(obj, method):
    wrapper = collections.defaultdict(lambda: 'Unavailable')

    try:
        func = getattr(obj, method)
        original_dict = await func()
...
async def run(parsed_args):
    async with aiohttp.ClientSession() as websession:
        v = await get_vehicle(websession, parsed_args)

        status = await wrap_unavailable(v, 'battery_status')
        ...
        try:
            charge_mode = await v.charge_mode()
        except requests.RequestException:
            charge_mode = 'Unavailable'

        mileage = await wrap_unavailable(v, 'mileage')
        ...
        location = await wrap_unavailable(v, 'location')
        ...
        hvac = await wrap_unavailable(v, 'hvac_status')
